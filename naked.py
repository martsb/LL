...
# Trying to make a connection with database
def get_cursor():
        global connection
        try:
...
def mysql_insert_ast_into_db(create_date, hazardous, name, url, diam_min, diam_max, ts, dt_utc, dt_local, speed, distance, ast_id):
        ...
        try:
                ...
                pass
# Checking and adding asteroid into DB, if it is not there already
...
# Defining where main loop starts
if __name__ == "__main__":
  ...
  # Opening connection to mysql DB
    ...
    if r.status_code == 200:
            ...
            # Parsing a response as a json structure
            json_data = json.loads(r.text)

            # Initializing arrays for safe and hazardous asteroids
            ast_safe = []
            ast_hazardous = []

            # Checking if matches with the one in json data
            if 'element_count' in json_data:
                    ast_count = int(json_data['element_count'])
                    logger.info("Asteroid count today: " + str(ast_count))
                    # If there are asteriods, getting their data and writing them into the database
                    if ast_count > 0:
					    ...
					    # Getting id of asteroid
                                            tmp_ast_id = val['id']
                                            # Getting estimated diameter for asteroids and if it is harzardous
                                            ...
                                            # Getting asteroid stats and checking if they are close to Earth
                                            ...
                                            if len(val['close_approach_data']) > 0:
                                                    if 'epoch_date_close_approach' and 'relative_velocity' and 'miss_distance' in val['close_approach_data'][0]:
                                                            ...
                                                            # Checking if asteroids velocity is known
                                                            if 'kilometers_per_hour' in val['close_approach_data'][0]['relative_velocity']:
                                                                    tmp_ast_speed = int(float(val['close_approach_data'][0]['relative_velocity']['kilometers_per_hour']))
                                                            else:
                                                                    tmp_ast_speed = -1
                                                            # Checking if it is known, how close to Earth the asteroid will get
                                            ...
                                            # Printing out info about the asteroid
                                            logger.info("------------------------------------------------------- >>")
                    ...
                    # If there were no asteroids found today printing that out
                    else:
            ...
            # Sorting hazardous asteroids, if there are some
    ...
    # Printing error message, if connection failed
    else:
